...
// NMEA sentences have at most ~20 fields; cap to a fixed array so this
// never touches the heap allocator. Individual field strings stay within
// libstdc++'s small-string-optimization (no heap), but a std::vector here
// would reallocate on growth — and this function runs continuously on every
// GPS sentence, often right when the heap is most fragmented (e.g. just
// after a WiFi/CAT connect), which previously caused a std::bad_alloc abort.
constexpr size_t kMaxCsvFields = 20;

size_t split_csv(const std::string& s, std::string (&out)[kMaxCsvFields]) {
  size_t count = 0;
  size_t start = 0;
  while (start <= s.size() && count < kMaxCsvFields) {
    size_t comma = s.find(',', start);
    if (comma == std::string::npos) {
      out[count++] = s.substr(start);
      break;
    }
    out[count++] = s.substr(start, comma - start);
    start = comma + 1;
  }
  return count;
}
...
bool parse_sentence(const std::string& raw_line) {
  ...
  std::string parts[kMaxCsvFields];
  size_t part_count = split_csv(payload, parts);
  if (part_count == 0) return false;

  bool recognized = false;
  const std::string& type = parts[0];
  if (type.size() >= 3 && type.compare(type.size() - 3, 3, "RMC") == 0 && part_count >= 10) {
    recognized = true;
    if (parts[2] == "A") {
  ...
  } else if (type.size() >= 3 && type.compare(type.size() - 3, 3, "GGA") == 0 && part_count >= 8) {
    recognized = true;
