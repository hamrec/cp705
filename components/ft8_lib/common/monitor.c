...
// Sized for FT8 at 6kHz: max_blocks=93, time_osr=1, freq_osr=1, num_bins=433.
// time_osr was 2 (160,538B... 80,538B); reduced to 1 to reclaim ~40KB of the
// no-PSRAM board's RAM so WiFi TX buffers and general heap have room (TX audio
// was failing with ENOMEM). MUST stay in lockstep with g_time_osr in main.cpp —
// if g_time_osr > 1, runtime wf_bytes exceeds this and falls back to a (likely
// failing) malloc. Cost: ~1-2dB weak-signal/off-timing decode sensitivity.
#define WF_STATIC_SIZE  (93 * 1 * 1 * 433)  // 40,269 bytes
